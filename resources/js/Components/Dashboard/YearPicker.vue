...
const currentYear = ref(Number(props.modelValue) || new Date().getFullYear());

watch(() => props.modelValue, (newVal) => {
    if (newVal) {
        currentYear.value = Number(newVal);
    }
});

const displayValue = computed(() => {
    if (!props.modelValue) return 'Selecionar Ano';
    return `Ano de ${currentYear.value}`;
});
...
<template>
    <div class="relative inline-block text-left w-full sm:w-[240px]" ref="pickerContainer">
        ...
        <div 
            v-if="isOpen"
            class="absolute top-[calc(100%+8px)] right-0 w-[240px] bg-white dark:bg-slate-800 rounded-2xl shadow-xl z-[100] border border-slate-100 dark:border-slate-700 overflow-hidden animate-slide-up"
        >
            <div class="p-4">
                <!-- Year Selector -->
