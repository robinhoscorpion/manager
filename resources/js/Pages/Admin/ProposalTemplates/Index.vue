...
const processHtmlTags = (html) => {
    const mockData = {
        ...
        '${PROPOSTA_RESUMO_ENTRADA}': '5x de R$ 1.000,00 no Cartão de Crédito (Início: 10/08/2026)<w:br/>+ 5x de R$ 1.000,00 no Boleto (Início: 10/01/2027)',
        '${PROPOSTA_SALDO}': 'R$ 35.000,00',
        '${PROPOSTA_RESUMO_SALDO}': '20x de R$ 1.000,00 no Boleto Bancário (Início: 10/02/2027)<w:br/>+ 15x de R$ 1.000,00 no PIX (Início: 10/10/2028)',
        '${PROPOSTA_RESUMO_TAXA}': '1x de R$ 125,00 no PIX (Início: 16/07/2026)<w:br/>+ 1x de R$ 125,00 no Dinheiro (Início: 16/08/2026)',
        '${PROPOSTA_RESUMO_MANUTENCAO}': 'Anual de R$ 600,00 no Boleto Bancário<w:br/>+ Anual de R$ 600,00 no Cartão de Crédito',
        '${PROPOSTA_DATA}': new Date().toLocaleDateString('pt-BR'),
        '${PROPOSTA_DATA_EXTENSO}': '16 de Julho de 2026',
        '${PROPOSTA_VIGENCIA}': '5 (cinco) anos',
        ...
        '${USUARIO_IMPRESSAO}': 'Administrador',
    };
...
const openPreview = async (item) => {
    ...
    if (item.original_filename) {
        ...
        try {
            ...
            const response = await axios.get(route('admin.proposal_templates.test_print', item.id), {
            ...
            parsedPreviewContent.value = `<div class="text-center text-red-500 py-10">
                ...
                <a href="${route('admin.proposal_templates.test_print', item.id)}" class="underline mt-4 inline-block font-bold">Baixar Arquivo Original</a>
            </div>`;
        } finally {
...
const executeDelete = () => {
    if (itemToDelete.value) {
        form.delete(route('admin.proposal_templates.destroy', itemToDelete.value.id), {
            onSuccess: () => {
                    ...
                    <div class="px-6 py-5 border-b border-slate-200/50 dark:border-slate-800/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div class="flex items-center gap-4 w-full sm:w-auto justify-center sm:justify-start">
                            ...
                            <div class="text-center sm:text-left">
                                <h2 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight leading-none mb-1">Modelos de Proposta</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6 pb-12">
                    ...
                    >
                        ...
                        <div class="p-6 flex flex-col flex-1 relative z-20">
                            ...
                            <div class="flex items-start gap-4 mb-4">
                                ...
                                <div class="flex flex-col pt-1">
                                    <span 
                                        class="text-[9px] font-black uppercase tracking-widest mb-1 text-slate-400"
                                    >
                                        Modelo
                                    </span>
                        ...
                        <div class="mt-auto border-t border-slate-100 dark:border-slate-800/60 p-3 bg-slate-50/50 dark:bg-slate-900/50 flex justify-end gap-1 relative z-20">
                            ...
                            <a v-if="item.original_filename" :href="route('admin.proposal_templates.test_print', item.id)" class="flex items-center justify-center w-8 h-8 rounded-lg text-slate-400 hover:text-purple-500 hover:bg-purple-50 dark:hover:bg-purple-500/10 transition-colors" title="Testar Impressão (Baixar Word com tags preenchidas)">
                            ...
                            <a v-if="item.original_filename" :href="route('admin.proposal_templates.download_pdf', item.id)" class="flex items-center justify-center w-8 h-8 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors" title="Gerar PDF (Exige LibreOffice no Servidor)">
            ...
            <div class="bg-white dark:bg-[#0f1219] rounded-[20px] overflow-hidden flex flex-col max-h-[90vh]">
                <div class="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between flex-shrink-0">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">{{ editingItem ? 'Editar Proposta' : 'Novo Proposta' }}</h2>
                    </div>
                ...
                <form @submit.prevent="submit" class="p-6 space-y-6 overflow-y-auto flex-1 custom-scrollbar bg-slate-50 dark:bg-transparent">
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        <div class="col-span-3 space-y-1.5">
                            <label class="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider px-1 text-left block">Título do Proposta</label>
                            <input v-model="form.name" type="text" required class="w-full bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-900 dark:text-white text-sm outline-none focus:border-brand-green/50 focus:ring-1 focus:ring-brand-green/50 transition-all shadow-sm" :class="{'border-red-500 focus:border-red-500 focus:ring-red-500/50': form.errors.name}" placeholder="Ex: Proposta Padrão de Time-Sharing">
                            <p v-if="form.errors.name" class="text-xs text-red-500 mt-1 px-1 font-medium">{{ form.errors.name }}</p>
                        </div>
                        <div class="flex items-end">
                            <div class="w-full flex items-center gap-3 bg-white dark:bg-slate-800/50 p-2.5 px-4 rounded-xl border border-slate-200 dark:border-slate-700 h-[42px] shadow-sm">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <span class="ml-3 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Ativo</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center justify-between px-1">
                            <label class="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Arquivo do Proposta (Word .docx)</label>
                        ...
                        <p v-if="form.errors.file" class="text-xs text-red-500 mt-1 px-1 font-medium">{{ form.errors.file }}</p>
                    </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-6 mt-4">
                            ...
                            <div>
                                ...
                                <ul class="text-[10px] text-slate-600 dark:text-slate-400 font-mono flex flex-col">
                                    ...
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${PROPOSTA_USO_INICIAL}</span> <span class="font-sans text-slate-400 truncate pl-2">2027</span></li>
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${PROPOSTA_VIGENCIA}</span> <span class="font-sans text-slate-400 truncate pl-2">12 Meses</span></li>
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${PROPOSTA_ENTRADA}</span> <span class="font-sans text-slate-400 truncate pl-2">R$ 10.000,00</span></li>
                                    <li class="flex flex-col p-2.5 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors bg-brand-green/5 rounded-t-lg">
                                        <span class="text-brand-green font-bold">${PROPOSTA_RESUMO_ENTRADA}</span>
                                        <span class="font-sans text-slate-500 text-[10px] mt-1.5 leading-relaxed bg-white dark:bg-slate-900/50 p-2 rounded border border-brand-green/20">5x R$ 1.000,00 no Cartão (Início: 10/08)<br>+ 5x R$ 1.000,00 no Boleto (Início: 10/01)</span>
                                    </li>
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${PROPOSTA_SALDO}</span> <span class="font-sans text-slate-400 truncate pl-2">R$ 35.000,00</span></li>
                                    <li class="flex flex-col p-2.5 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors bg-brand-green/5">
                                        <span class="text-brand-green font-bold">${PROPOSTA_RESUMO_SALDO}</span>
                                        <span class="font-sans text-slate-500 text-[10px] mt-1.5 leading-relaxed bg-white dark:bg-slate-900/50 p-2 rounded border border-brand-green/20">20x R$ 1.000,00 no Boleto (Início: 10/02)<br>+ 15x R$ 1.000,00 no PIX (Início: 10/10)</span>
                                    </li>
                                    <li class="flex flex-col p-2.5 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors bg-brand-green/5">
                                        <span class="text-brand-green font-bold">${PROPOSTA_RESUMO_TAXA}</span>
                                        <span class="font-sans text-slate-500 text-[10px] mt-1.5 leading-relaxed bg-white dark:bg-slate-900/50 p-2 rounded border border-brand-green/20">1x R$ 125,00 no PIX (Início: 16/07)<br>+ 1x R$ 125,00 no Dinheiro (Início: 16/08)</span>
                                    </li>
                                    <li class="flex flex-col p-2.5 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors bg-brand-green/5 rounded-b-lg mb-2">
                                        <span class="text-brand-green font-bold">${PROPOSTA_RESUMO_MANUTENCAO}</span>
                                        <span class="font-sans text-slate-500 text-[10px] mt-1.5 leading-relaxed bg-white dark:bg-slate-900/50 p-2 rounded border border-brand-green/20">Anual R$ 600,00 no Boleto<br>+ Anual R$ 600,00 no Cartão</span>
                                    </li>
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${PROPOSTA_VIGENCIA}</span> <span class="font-sans text-slate-400 truncate pl-2">5 (cinco) anos</span></li>
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${PROPOSTA_DATA}</span> <span class="font-sans text-slate-400 truncate pl-2">16/07/2026</span></li>
                                    <li class="flex items-center justify-between p-2 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${PROPOSTA_DATA_EXTENSO}</span> <span class="font-sans text-slate-400 truncate pl-2">16 de Julho de 2026</span></li>
                            ...
                            <!-- Equipe de Vendas -->
                            <div class="md:col-span-1 lg:col-span-3 mt-2">
                                <h4 class="text-[13px] font-bold text-slate-700 dark:text-slate-300 mb-3 border-b-2 border-brand-green/30 pb-2 flex items-center gap-2">
                                ...
                                <ul class="text-[10px] text-slate-600 dark:text-slate-400 font-mono grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4">
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${VENDEDOR_NOME}</span> <span class="font-sans text-slate-400 truncate pl-2">Marcos Sales</span></li>
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${PROMOTOR_NOME}</span> <span class="font-sans text-slate-400 truncate pl-2">Lucas Silva</span></li>
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${CONSULTOR_NOME}</span> <span class="font-sans text-slate-400 truncate pl-2">Mariana Costa</span></li>
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${SUPERVISOR_NOME}</span> <span class="font-sans text-slate-400 truncate pl-2">Carlos Alberto</span></li>
                                    <li class="flex items-center justify-between p-2 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${GERENTE_NOME}</span> <span class="font-sans text-slate-400 truncate pl-2">Ana Beatriz</span></li>
                                </ul>
                            </div>
                            ...
                            <div class="md:col-span-1 lg:col-span-3 mt-2">
                                ...
                                <ul class="text-[10px] text-slate-600 dark:text-slate-400 font-mono grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4">
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${DATA_ATUAL}</span> <span class="font-sans text-slate-400 truncate pl-2">17/07/2026</span></li>
                                    <li class="flex items-center justify-between p-2 border-b border-slate-100 dark:border-slate-800 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${HORA_ATUAL}</span> <span class="font-sans text-slate-400 truncate pl-2">14:30:00</span></li>
                                    <li class="flex items-center justify-between p-2 hover:bg-brand-green/5 transition-colors"><span class="text-brand-green font-bold">${USUARIO_IMPRESSAO}</span> <span class="font-sans text-slate-400 truncate pl-2">Administrador</span></li>
                                </ul>
                            </div>
                        </div>
                    </div>
        ...
        <Modal :show="showPreviewModal" @close="showPreviewModal = false" maxWidth="7xl">
            <div v-if="previewItem" class="bg-white dark:bg-[#0f1219] border border-slate-200 dark:border-slate-800 rounded-[20px] relative overflow-hidden animate-in fade-in zoom-in-95 duration-200 shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between flex-shrink-0 relative z-10">
                    ...
                    <div class="flex items-center gap-3">
                        <button @click="showPreviewModal = false; openEditModal(previewItem)" class="flex items-center gap-2 text-[10px] font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-4 py-2 rounded-xl hover:bg-emerald-500 hover:text-white transition-colors border border-slate-200 dark:border-slate-700 hover:border-emerald-500 uppercase tracking-widest">
        ...
        <Modal :show="showDeleteConfirmModal" @close="showDeleteConfirmModal = false" maxWidth="md">
            <div class="p-6 bg-white dark:bg-[#0f1219]">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 text-center mb-8 px-4 leading-relaxed">
                    ...
                    <span class="text-red-500 font-bold block mt-3">Esta ação não pode ser desfeita.</span> Todos os  vinculados perderão este modelo.
                </p>
