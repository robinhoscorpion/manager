    ...
    <AuthenticatedLayout>
                        ...
                        <div class="p-6 space-y-4 flex-1">
                            <div class="flex justify-between items-start">
                                ...
                                <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <a v-if="item.content" :href="route('admin.complimentary_items.pdf', item.id)" target="_blank" class="p-1.5 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-500 hover:text-slate-700 dark:hover:text-white transition-colors" title="Visualizar em PDF">
                                    ...
                                    <button v-if="item.content" @click="openPreview(item)" class="p-1.5 bg-slate-50 dark:bg-slate-800/50 hover:bg-brand-green/10 rounded-lg text-slate-500 hover:text-brand-green transition-colors" title="Visualizar">
                                    ...
                                    <button @click="deleteItem(item.id)" class="p-1.5 bg-slate-50 dark:bg-slate-800/50 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg text-slate-500 hover:text-red-500 transition-colors" title="Remover">
                ...
                <form @submit.prevent="submit" class="p-6 space-y-5 overflow-y-auto flex-1 bg-slate-50 dark:bg-transparent">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center justify-between px-1">
                            ...
                            <span class="text-[9px] text-slate-500 uppercase tracking-widest">Editor Visual</span>
                        </div>
                    ...
                    <div class="bg-white dark:bg-[#1a1f2e]/50 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-sm">
        ...
        <Modal :show="showPreviewModal" @close="showPreviewModal = false" maxWidth="7xl">
            <div v-if="previewItem" class="bg-white dark:bg-[#0f1219] rounded-[20px] overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <!-- Browser Shell -->
                <div class="p-8 overflow-y-auto flex-1 preview-content bg-white dark:bg-slate-50" v-html="parsedPreviewContent"></div>
            </div>
        </Modal>
    </AuthenticatedLayout>
</template>
