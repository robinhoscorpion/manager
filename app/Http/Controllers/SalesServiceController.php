...
class SalesServiceController extends Controller
{
    ...
    public function pdfProposta(SalesService $service)
    {
        ...
        $mockData = [
            ...
            '${PROPOSTA_USO_INICIAL}' => date('Y') + 1, // Exemplo, já que não há campo específico na tabela
            '${PROPOSTA_VIGENCIA}' => $proposal->product?->validity ?? ($proposal->product?->duration ? $proposal->product->duration . ' Meses' : ''),
